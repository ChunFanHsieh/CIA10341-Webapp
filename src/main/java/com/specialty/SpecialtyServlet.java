	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("insert".equals(action)) {
			...
			try {
				...
				if (!errorMsgs.isEmpty()) {
					req.setAttribute("specialtyVO", specialtyVO);
					RequestDispatcher failureView = req.getRequestDispatcher("/back-end/specialty/addSpecialty.jsp");
					failureView.forward(req, res);
					return;
				}
				...
				String url = "/back-end/specialty/listAllSpecialty.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url);
				successView.forward(req, res);
		...
		if ("delete".equals(action)) {
			...
			try {
				...
				RequestDispatcher failureView = req.getRequestDispatcher("/back-end/specialty/listAllSpecialty.jsp");
				failureView.forward(req, res);
			}
		}

		if ("getOne_For_Display_By_Name".equals(action)) {
			List<String> errorMsgs = new LinkedList<>();
			req.setAttribute("errorMsgs", errorMsgs);
			...
			try {
				...
				if (specName == null || specName.trim().length() == 0) {
					errorMsgs.add("請選擇占卜項目名稱");
				}

				SpecialtyService specialtySvc = new SpecialtyService();
				SpecialtyVO specialtyVO = specialtySvc.getSpecialtyByName(specName);
				if (specialtyVO == null) {
					errorMsgs.add("查無資料");
				}

				if (!errorMsgs.isEmpty()) {
					RequestDispatcher failureView = req.getRequestDispatcher("/back-end/specialty/select_page.jsp");
					failureView.forward(req, res);
					return;
				}

				req.setAttribute("specialtyVO", specialtyVO);
				String url = "/back-end/specialty/listOneSpecialty.jsp";
				RequestDispatcher successView = req.getRequestDispatcher(url);
				successView.forward(req, res);
